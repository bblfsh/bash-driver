pipeline {
  agent {
    kubernetes {
      label 'bash-driver-bblfsh-performance'
      defaultContainer 'bash-driver-bblfsh-performance'
      yaml """
spec:
  nodeSelector:
  ...
  containers:
  - name: bash-driver-bblfsh-performance
...
"""
  ...
  environment {
    DRIVER_LANGUAGE = "bash"
    DRIVER_REPO = "https://github.com/bblfsh/bash-driver.git"
